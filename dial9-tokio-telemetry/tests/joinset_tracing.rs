...
//! - `TelemetryHandle::spawn_in_joinset` / mirror on runtime handle
...
/// `spawn_in_joinset` emits wake events AND marks `TaskSpawn` instrumented,
/// AND propagates the caller location via `#[track_caller]`.
#[test]
fn spawn_in_joinset_emits_wakes_and_marks_instrumented() {
    let dir = tempfile::tempdir().unwrap();
    let trace_path = dir.path().join("trace.bin");
    let writer = RotatingWriter::single_file(&trace_path).unwrap();
    let (runtime, guard) = build_traced_runtime(writer);
    ...
    runtime.block_on(async move {
        let mut set: JoinSet<()> = JoinSet::new();
        handle.spawn_in_joinset(&mut set, async move {
            *id_w.lock().unwrap() = tokio::task::try_id().map(TaskId::from);
            tokio::task::yield_now().await;
        });
        while set.join_next().await.is_some() {}
    ...
    assert!(saw_wake, "expected WakeEvent for spawn_in_joinset task");

    // Verify spawn_loc resolves and points to this test file (proves
    // `#[track_caller]` propagation works through `spawn_in_joinset`).
...
/// `RuntimeTelemetryHandle::spawn_in_joinset` targets the correct runtime
/// even when called from outside any runtime context.
#[test]
fn runtime_handle_spawn_in_joinset_targets_correct_runtime() {
    ...
    handle_a.spawn_in_joinset(&mut set_a, async {
        tokio::task::yield_now().await;
        std::thread::current().name().unwrap_or("?").to_string()
    });

    let mut set_b: JoinSet<String> = JoinSet::new();
    handle_b.spawn_in_joinset(&mut set_b, async {
        tokio::task::yield_now().await;
        std::thread::current().name().unwrap_or("?").to_string()
    });
