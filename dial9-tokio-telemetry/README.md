...
DIAL9_SCHEDULE_PROFILE_ENABLED=false
```

In this mode, dial9 does not install Tokio runtime hooks. APIs that depend on
Tokio hook-provided runtime context will not observe that context, and callbacks
registered with `with_tokio_hooks` are not installed.
